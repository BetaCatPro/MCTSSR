...
def get_mix_data(model, m_data1, m_data2, lam):
    un_target1 = model(m_data1)
    un_target2 = model(m_data2)
    mix_target = lam * un_target1 + (1 - lam) * un_target2
    mix_data = lam * m_data1 + (1 - lam) * m_data2
    mix_data_target = model(mix_data)

    return mix_target, mix_data_target


def train_reg_net(epoch, data_loader, model, optimizer, device, total):
    """
    supervised loss + weight * consistency loss
    """
    loss = 0
    train_loss = 0
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        # supervised loss
        model_supervised_loss = smooth_l1_loss(model(lab_data), lab_data_target)

        mix_target, mix_data_target = get_mix_data(model, lab_unlabeled_data1, lab_unlabeled_data2, lam)

        # 视图内一致性正则
        inner_consistency_loss = smooth_l1_loss(mix_target, mix_data_target)

        # total loss
        alpha_weight = 1.0 * sigmoid_ramp_up(epoch + 1, 2/total)
...
def validate(data, model, device):
    ...
    with torch.no_grad():
        ...
        loss = mse_loss(model(val_data), targets)
