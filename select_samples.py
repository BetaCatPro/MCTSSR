...
def select_sim_samples(data_name, tau, in_channels, scale, data_size=0, s_strategy='metric'):
    """
    使用度量网络选择相似无标记样本
    :param data_name:
    :param tau: 衰减阈值
    :param in_channels: 输入通道
    :param scale: 划分标记数据比例
    :param data_size: 选择的新样本的大小
    :param s_strategy: 样本挑选策略，default: 度量选择
    ...
    if s_strategy == 'metric':
        ...
        # random select 50 instances
        final_selected_list = unlabeled_data.sample(50)
        unlabeled_data = unlabeled_data.reset_index(drop=True).drop(final_selected_list.index)
